...
def run(weights='yolov5s.pt',  # model.pt path(s)
    ...
    classify, pt, onnx = False, w.endswith(
        '.pt'), w.endswith('.onnx')  # inference type
...
def main(opt):
    ...
    # Print detected_classes as an array
    with open("detected.txt", "w") as output:
        output.write(str(detected_classes))
